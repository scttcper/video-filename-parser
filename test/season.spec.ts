...
describe('season', () => {
  ...
  const dayEpisodeCases: Array<[string, string, Date]> = [
    ['Jimmy Fallon 2019 10 23 Michael Douglas 1080p HEVC x265-MeGusta', 'Jimmy Fallon', new Date(2019, 9, 23)],
    ['NFL.2019.10.06.Chicago.Bears.vs.Oakland.Raiders.Highlights.WEB.H264-LEViTATE', 'NFL', new Date(2019, 9, 6)],
  ];
  ...
  it.each(animeSpecialCases)('should handle anime special episodes "%s"', (postTitle, title, episodeNumber) => {
    const result = parseSeason(postTitle) as Season;
    expect(result.seriesTitle).toBe(title);
    expect(result.episodeNumbers).toHaveLength(1);
    expect(result.episodeNumbers[0]).toBe(episodeNumber);
  ...
  it.each(animeRecapCases)('should handle anime recap episodes "%s"', (postTitle, title, specialEpisodeNumber) => {
    const result = parseSeason(postTitle) as Season;
    expect(result.seriesTitle).toBe(title);
    expect(result.episodeNumbers).toHaveLength(1);
  ...
  // ];
  // it.each(animePackCases)('should parse anime season packs "%s"', (postTitle, title, seasonNumber) => {
  //   const result = parseSeason(postTitle) as Season;
  //   expect(result.seriesTitle).toBe(title);
  //   expect(result.fullSeason).toBe(true);
  //   expect(result.seasons).toHaveLength(1);
  //   expect(result.seasons[0]).toBe(seasonNumber);
  // });
